  ...
  return (
    ...
    { session ? <Wrapper>
      ...
      <Form onSubmit={handleSubmit} enctype="multipart/form-data">
        ...
        <Input type="text" placeholder="Not more than 10 words" value={description} onChange={(e) => setDescription(e.target.value)} required />

        <Label htmlFor="description">About</Label>
        <TextArea value={about} placeholder="Not above 45 words not below 40" onChange={(e) => setAbout(e.target.value)} required />
        ...
        <Input type="number" value={price} placeholder="Enter 0 if its free" onChange={(e) => setPrice(e.target.value)} required />

        <Label htmlFor="whatYouWillLearn">What You Will Learn</Label>
        <TextArea type="text" placeholder="List benefits separated by !" value={whatYouWillLearn} onChange={(e) => setWhatYouWillLearn(e.target.value)} />

        <Label htmlFor="content">Course Content</Label>
        <TextArea placeholder="List topics separated by !" value={content} onChange={(e) => setContent(e.target.value)} />
        {/* // img upload */}
        <ImgDiv>
          <Label htmlFor="image"> Course Image</Label>
          <VidInput type="file" accept="image/*" onChange={(e) => setImage(e.target.files[0])} />
        </ImgDiv>
          ...
          {lessons.map((lesson, lessonIndex) => (
            <div key={lessonIndex}>
              ...
              {lesson.videos.map((video, videoIndex) => (
                <div key={videoIndex}>
                  <VidLabel htmlFor={`lesson-video-title-${lessonIndex}-${videoIndex}`}>Video Title</VidLabel>
                  <VidInput type="text" value={video.title} onChange={(e) => updateVideoTitle(e, lessonIndex, videoIndex)} /><br />

                  <VidLabel htmlFor={`lesson-video-link-${lessonIndex}-${videoIndex}`}>Video Link</VidLabel>
                  <VidInput type="text" value={video.link} onChange={(e) => updateVideoLink(e, lessonIndex, videoIndex)} /><br />
                </div>
              ))}

              <Button type="button" onClick={() => handleAddVideo(lessonIndex)}>Add Video</Button>
